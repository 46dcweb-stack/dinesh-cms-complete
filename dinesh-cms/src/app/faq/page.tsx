...
export const metadata = {
    title: "FAQ",
    description: "Frequently asked questions about Dinesh Koyyalamudi and FourSix46.",
};

export default async function FAQPage() {
    const fbFaq = await getPublishedFaq();

    // Build groups from Firebase or fall back to static
    const faqGroupsData = fbFaq.length > 0
        ? Object.entries(
            fbFaq.reduce((acc: any, item: any) => {
                const cat = item.category ?? "General";
                if (!acc[cat]) acc[cat] = [];
                // Remap Firebase fields { question, answer } → { q, a } expected by FAQGrid
    ...
    return (
        <div className="pt-20 lg:pt-12 pb-24">
            ...
            {/* Quick FAQ summary at the bottom */}
