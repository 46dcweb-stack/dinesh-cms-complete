  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-[300px] md:auto-rows-[250px]">
      {images.map((image, idx) => {
        ...
        if (!src) return null;
        return (
          <motion.div key={image.id || idx} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.1 }}
            className={`relative group overflow-hidden rounded-3xl border border-white/5 glass-card ${image.span ?? ""}`}
          >
            <Image src={src} alt={image.title} fill className="object-cover grayscale brightness-75 group-hover:grayscale-0 group-hover:scale-105 group-hover:brightness-100 transition-all duration-700 ease-out" />
