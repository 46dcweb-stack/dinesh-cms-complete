...
export default function HorizontalNewsroom({
    posts,
    eyebrow = "Thought Pulse",
    ...
    const normalText = titleWords.slice(0, -2).join(" ");
    const gradientText = titleWords.slice(-2).join(" ");

    return (
        <section
            ref={targetRef}
            className="relative h-[300vh] bg-brand-dark/20"
        >
            <div className="sticky top-0 h-screen flex items-center overflow-hidden">
                ...
                <div className="absolute top-10 md:top-20 left-0 w-full md:w-auto md:left-20 max-w-2xl px-6 text-center md:text-left z-20">
                    <span className="text-brand-primary font-medium tracking-[0.2em] text-[10px] md:text-xs uppercase block mb-4 font-mono">
                        {eyebrow}
                    </span>
                ...
                <motion.div
                    style={{ x }}
                    className="flex gap-6 md:gap-8 px-6 md:px-20 mt-16 md:mt-20"
                >
